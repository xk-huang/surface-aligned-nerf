...
class Dataset(data.Dataset):
    ...
    def prepare_input(self, i):
        ...
        vertices_path = os.path.join(self.data_root, cfg.vertices,
                                     '{}.npy'.format(i))
        xyzw = np.load(vertices_path).astype(np.float32)

        # obtain the original bounds for point sampling
        min_xyz = np.min(xyzw, axis=0)
        max_xyz = np.max(xyzw, axis=0)
        if cfg.big_box:
            min_xyz -= 0.05
            max_xyz += 0.05
        else:
        ...
        # min_xyz[[0,1]] -= 0.05
        # max_xyz[[0,1]] += 0.05
        # can_bounds_big = np.stack([min_xyz, max_xyz], axis=0)
        ...
        xyz = np.dot(xyzw - Th, R)

        shapes = params['shapes']
        poses = params['poses'].astype(np.float32).squeeze()
        ...
        return coord, out_sh, can_bounds, bounds, Rh, Th, xyz, xyzw, poses, shapes
    ...
    def __getitem__(self, index):
        ...
        if self.human in ['CoreView_313', 'CoreView_315']:
            ...
            frame_index = i
        coord, out_sh, can_bounds, bounds, Rh, Th, xyz, xyzw, poses, shapes = self.prepare_input(
            i)

        rgb, ray_o, ray_d, near, far, coord_, mask_at_box, mask_at_box_eval = if_nerf_dutils.sample_ray_h36m(
        ...
        ret = {
            ...
            'verts_world': xyzw,
            'poses': poses, 
            'mask_at_box_eval': mask_at_box_eval
        }

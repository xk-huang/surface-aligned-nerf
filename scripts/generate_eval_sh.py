...
for _time in [5*60, 10*60, 60*60*2]:
    ...
    for i, j in enumerate('313 315 377 386 387 390 392 393 394 11 1 5 6 7 8 9'.split(' ')):
        ...
        if int(j) > 300:
            ...
            print(f'id={j}; python run.py --type evaluate --cfg_file configs/zju_mocap_exp/multi_view_${{id}}.yaml exp_name zju_mocap/{_time}/test_${{id}} gpus [{i%num_gpus}] test.epoch {weight_epoch} &')
        else:
            weights_dir = f"{base_dir}/h36m/{_time}/test_{j}/"
            weight_epoch = find_largest_epoch(weights_dir)
            print(f'id={j}; python run.py --type evaluate --cfg_file configs/h36m_exp/latent_xyzc_s${{id}}p.yaml exp_name h36m/{_time}/test_${{id}} gpus [{i%num_gpus}] test.epoch {weight_epoch} &')
    print('wait')